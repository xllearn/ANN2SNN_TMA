...
class QCFS(nn.Module):
    def __init__(self, up=8., t=32):
        ...
        # self.up = nn.Parameter(torch.zeros(1, channel_in, 1, 1) + up, requires_grad=True)
        self.t = t

    def _f(self, x):
        """实现 f(x) = (x + sqrt(x^2 + 1))/2 函数"""
        return (x + torch.sqrt(x**2 + 0.01)) / 2

    def forward(self, x):
        # 计算正约束的阈值
        threshold = self._f(self.up)
